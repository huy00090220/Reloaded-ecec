...
logger = logging.getLogger("SwiftDLC loader")
...
def find_java_executable(user_path: Optional[str] = None) -> Optional[str]:
    """
    Find java in user settings, JAVA_HOME, or PATH.
    Priority: user_path > JAVA_HOME > PATH
    """
    # 1. User specified path
    if user_path and os.path.exists(user_path):
        logger.info(f"Found java in user settings: {user_path}")
        return user_path
    ...
    logger.warning("Java not found in user path, JAVA_HOME, or PATH")
    return None
...
class LauncherWindow(QMainWindow):
    def __init__(self):
        ...
        # Note: Java path is now loaded from config, but we don't call find_java_executable here
        self.java_path = self.config.get("java_path", "") # "" instead of None for easy check
    ...
    def _init_ui(self):
        ...
        desc = QLabel("Лучший чит клиент для игры в Minecraft."
                      "Тех.Поддержка- @aibertIIIk")
        ...
        # Buttons area (Logs / Settings) - REMOVED Verify/Reinstall
        btns_frame = QFrame()
        ...
        # self.btn_verify = QPushButton("Verify / Reinstall") # REMOVED
        # self.btn_verify.setProperty("class", "small") # REMOVED
        # self.btn_verify.setFixedHeight(36) # REMOVED
        # self.btn_verify.clicked.connect(self.on_verify_clicked) # REMOVED
        # btns_layout.addWidget(self.btn_verify) # REMOVED
        ...
        btns_layout.addWidget(self.btn_logs, 1) # added stretch factor for balance
        ...
        btns_layout.addWidget(self.btn_settings, 1) # added stretch factor for balance
    ...
    def on_open_logs(self):
        if is_windows():
            try:
                ...
                QMessageBox.warning(self, "Log", f"Cannot open logs: {e}")
        else:
            QMessageBox.information(self, "Log", f"Log file: {os.path.abspath(LOG_FILE)}")

    def on_open_settings(self):
        # Используем self.java_path, который загружен из конфига, для диалога
    ...
    # REMOVED on_verify_clicked as requested
    ...
    # REMOVED verify_client and reinstall_client
        ...
        with self._bg_lock:
            ...
            if missing:
                ...
                if os.path.exists(CLIENT_ZIP_PATH):
                    ...
                    self.on_status_emit("Local archive not found. Downloading...")
                    # download archive
            ...
            # find java using the new priority logic (config > JAVA_HOME > PATH)
            java_exec = find_java_executable(self.java_path)
            ...
            try:
                # Список аргументов для записи в args.txt
                args_list = [
                    f"-Xmx{int(self.memory_gb)*1024}m",
                    ...
                    f"--gameDir {GAME_DIR}",
                    f"--assetsDir {os.path.join(GAME_DIR, 'swiftdlc', 'game', 'assets')}",
                    "--assetIndex 1.16",
                    f"--username {self.username}",
                    "--accessToken 0"
                ]
                
                # Сохраняем аргументы в args.txt (каждый аргумент на новой строке)
            ...
            # Формирование команды запуска с использованием файла аргументов
            ...
            try:
                ...
                out, err = proc.communicate(timeout=60) # Установим таймаут на 60 секунд
                ...
                if proc.returncode == 0:
                    ...
                    self.on_finished_emit(False, f"Launch failed with exit code {proc.returncode}. Check logs for details.")
            except subprocess.TimeoutExpired:
                 logger.error("Java process timed out.")
                 proc.kill()
                 self.on_finished_emit(False, "Launch failed: Java process timed out after 60 seconds.")
            except Exception as e:
...
# REMOVED VerifyDialog as requested
